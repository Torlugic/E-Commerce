...
export default function Navbar() {
  ...
  return (
    <div className="sticky top-0 z-50 shadow-sm">
      {/* main bar */}
      <header
        className="h-[var(--header-height)] bg-[var(--surface)] border-b border-[var(--border)]
      ...
      >
        <Link to={brand.links.home} className="flex items-center gap-[var(--space-sm)] hover:text-[var(--accent)]">
          ...
          <img src={brand.logo.light} alt={brand.logo.alt} className="h-8 w-auto block dark:hidden" />
          <img src={brand.logo.dark ?? brand.logo.light} alt={brand.logo.alt} className="h-8 w-auto hidden dark:block" />
          <span className="text-xl font-bold">{brand.siteName}</span>
        </Link>

        <nav className="flex gap-[var(--space-md)] items-center">
          <Link to={brand.links.products} className="hover:text-[var(--accent)]">
            Products
          </Link>
          <Link to={brand.links.cart} className="hover:text-[var(--accent)]">
            Cart
          </Link>
          <ThemeToggle />
          ...
          {user ? (
            <>
              <Link to="/profile" className="hover:text-[var(--accent)]">
                Profile
              </Link>
              <button onClick={logout} className="hover:text-[var(--accent)]" type="button">
                Logout
              </button>
            </>
          ) : (
            <>
              ...
              <Link to="/signup" className="hover:text-[var(--accent)]">
                Signup
              </Link>
            </>
          )}
